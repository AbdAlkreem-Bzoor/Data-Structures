...
public sealed class LinkedList<T>
{
    ...
    public T RemoveLast()
    {
        ...
        {
            var (_, previousNode) = GetNodeAt(Count - 1);
    ...
    public void RemoveAt(int index)
    {
        if (index < 0 || index >= Count)
        {
            throw new ArgumentOutOfRangeException(
                $"index is not in the linkedlist size range, should be from 0 to {Count - 1}");
        }
    ...
    private void SwapNeighbours(
        Node<T> leftNodePrevious, Node<T> leftNode, Node<T> rightNode,
        int left, int right)
    {
        if (left == 0)
        {
    ...
    public void Swap(int left, int right)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(left), left, "index is not in the linked-list size range");
        }
        
        if (right < 0 || right >= Count)
        {
            throw new ArgumentOutOfRangeException(nameof(right), right, "index is not in the linked-list size range");
        }
        ...
        if (right - left == 1)
        {
            SwapNeighbours(leftNodePrevious!, leftNode!, rightNode!, left, right);
        }
        else
        {
    ...
    public void Reverse()
    {
        Node<T>? previous = null, current = _head;
